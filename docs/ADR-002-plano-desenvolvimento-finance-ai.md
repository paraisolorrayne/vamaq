...
| **Auth do /admin** | ~~nginx sem `auth_basic`; `/admin` aberto na internet~~ → **fechado em 2026-07-26** (PR-B no ar): `/admin`→307 login, `/api/admin/*`→401 | **R3 resolvido** |
...
| **B** ✅ | Auth do `/admin`: `db/auth-schema.sql` (`users`/`sessions`, `must_change_password`), login em `/login` (Server Action + scrypt), **troca de senha obrigatória no 1º acesso** (`/trocar-senha`), `src/proxy.js` (matcher `/admin*`, `/api/admin*`, `/trocar-senha`), DAL (`verifySession`/`requireUser`/`requireRole`) + guarda de API (`requireApiRole`) em todas as rotas `/api/admin/*`, seed `scripts/seed-admin.mjs` (`--file` p/ lote, senha inicial compartilhada). **Deployado em produção 2026-07-26** (merge→main, schema aplicado, build, pm2 restart; verificado: `/acervo` 200, `/admin`→307, `/api/admin`→401, login real + força-troca OK). Semeado 1 admin (`devforaiagents@gmail.com`, senha inicial `2RcgYvMq@L`, troca no 1º acesso). **Pendente: e-mail de login definitivo + equipe** (rodar seed com `--file`) | A | **alto** — validado E2E em produção | — |
| **C** ✅ | Contrato do estoque: `status` aditivo (disponivel/reservado/vendido/inativo, §4.4-12), `setVehicleStatus` (+ despublica vendido/inativo), PATCH na API, "Excluir"→"Desativar"/"Reativar" na lista, role `vamaq_fin` só-leitura (`db/fin-blindagem.sql` + `scripts/setup-fin-role.sh` → `DATABASE_URL_FIN`), view `fin.v_vehicles`, **primeiro teste do repo** (`tests/blindagem.test.mjs`, `npm test` — 6 testes: CHECK de status, view, e financeiro não escreve). **Código feito e testado local 2026-07-26** (npm test 6/6; E2E: Desativar despublica e some do /acervo). Deploy: schema + `setup-fin-role.sh` na VPS | B | médio — validado por teste | — |

### Trilha 1 — Núcleo financeiro

| PR | Escopo | Depende | Risco ao core | Ordem de grandeza |
|---|---|---|---|---|
| **1** ✅ | `db/fin-schema.sql` no schema `fin` (dono `vamaq_fin`): `companies`, `chart_of_accounts`, `cost_centers`, `bank_accounts`, `contacts`, `transactions` (+`vehicle_id`→estoque, ON DELETE RESTRICT, dedupe `external_id`), view `fin.v_vehicle_margin`. Seed da Vamaq + **plano de contas de concessionária**. `src/lib/fin/db.js` (pool `DATABASE_URL_FIN`). **Deployado 2026-07-27** (fin: 19 contas, 4 CCs; blindagem confirmada em prod). Teste `tests/fin-schema.test.mjs` (5). Journal/partidas dobradas ficou de fora do núcleo (DRE/margem vêm de `transactions`) | C | baixo | ✅ |
| **2** 🟡 | **Parcial:** `src/lib/fin/calc.js` (DRE, margens, margem por veículo — ADR-001c §1) + `tests/fin-calc.test.mjs` (5) **feitos**. **Falta:** repositórios `src/lib/fin/repositories/*` + Route Handlers `/api/admin/financeiro/*` (CRUD lançamentos/contas/CCs/bancos/contatos) com autorização por papel `financeiro` | 1 | baixo | ~2 dias |
