...
foi digitado. O comportamento ao escolher um cliente depende de **criar** ou
**editar**, e são regras diferentes de propósito:

- **Criando** uma oportunidade nova, o que está no campo até a escolha é só o
  termo de **busca** ("Carl" para achar "Carlos Eduardo Mendes") — não é
  apelido de ninguém. Escolher o resultado **preenche `cliente_nome` com o
  nome do cadastro**, sempre, mesmo que o campo já tivesse algo digitado.
- **Editando** uma oportunidade que já existe, **o nome digitado NÃO é
  sobrescrito** pelo nome do cadastro — o vendedor conhece a pessoa por como
  digitou ("Carlinhos"), e o nome oficial do cadastro ("Carlos Eduardo
  Mendes") aparece à parte, junto do vínculo, tanto na tela da oportunidade
  quanto na ficha do cliente. Só preenche `cliente_nome` a partir do cadastro
  quando o campo ainda está vazio.

A tela `/vincular` (VincularForm.js) só existe para oportunidades que já
existem — é sempre o caso "editando" desta distinção, e nunca sobrescreve
`cliente_nome`. Ver fix-revisao2-report.md, itens 1 e 2, e a rodada de testes
que fechou esta entrega.
...
- **Formulário da oportunidade:** o campo de cliente vira busca. Digitando, mostra
  ...
  nome depende de criar ou editar — **criando**, o nome sempre vem do cadastro
  (o que estava no campo era só o termo de busca); **editando**, só vem do
  cadastro se o campo ainda estiver vazio, do contrário o que foi digitado
  permanece (ver a seção "Banco" acima). Não achou nada e o nome está
  preenchido: aparece **"Cadastrar «nome» como cliente novo"**, que cria o
  cadastro mínimo e já vincula.
